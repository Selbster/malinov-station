...
3. MUST: if the variable stores a component, use the base part of the component name (`ActiveMalinovRageComponent` -> `malinovRage`).
...
8. ID in the style `MobCarpChitter` with a clear domain prefix.
9. Fork ID with the `Malinov` prefix for unique fork content.
...
### Example 6: CamelCase for a prototype ID with an action suffix
...
Comment: `CamelCase` and an English action-name suffix are respected.

### Example 7: `Malinov` prefix in fork content

```yaml
- type: guideEntry
  id: MalinovAmmunition
  name: guide-entry-ammunition
```

Comment: The `_MalinovStation` identifier is clearly separate from the vanilla namespace.
