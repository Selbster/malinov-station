...
description: Creation, copying, renaming, migration and review of SS14 loadout prototypes (`roleLoadout`, `loadoutGroup`, `loadout`) in the `_MalinovStation` fork folder: naming, isolation from vanilla, transfer of mandatory equipment from `startingGear`, `startingGear` synchronization and processing of hidden jobs/departments. Use when changing job prototypes and any loadout system files.
---
...
3. This skill does not replace `ss14-upstream-maintenance`: when copying vanilla prototypes into the fork, still keep the changes isolated inside `_MalinovStation`.

## Source of truth

1. The source of truth for visible roles is the current loadout prototype graph of `_MalinovStation`.
...
4. Do not consider the loadout copy complete until all links from `_MalinovStation` roles are closed to local `_MalinovStation` prototypes.
...
2. Does the `_MalinovStation` role refer to vanilla or someone else's `loadoutGroup`?
   - Yes: copy it locally to `_MalinovStation` and rewrite the link.
...
1. `loadout`: `MalinovLoadout` + `ItemPrototypeId`.
2. `loadoutGroup`: `MalinovLoadoutGroup` + slot name or generic group name.
3. `roleLoadout`: `Job<RoleId>`.
4. If the same `ItemPrototypeId` is needed in two different contexts and without the suffix there will be a collision, add a short context tail: `MalinovLoadoutFlashlightLanternCargo`, `MalinovLoadoutFlashlightLanternEngineering`.
5. Do not leave new local IDs without the `Malinov` prefix.
...
3. Do not name the general group by the vanilla position if the role has already been renamed in `_MalinovStation`.

### File naming and placement

1. File names are `snake_case`.
2. The local path must mirror the original vanilla path inside `_MalinovStation`.
3. Don't put copies in technical dumps like `Imports/Malinov` if you can put them in a normal mirror path.
4. If the file refers to a specific role or department, use the name of the role and department, not a generic dump name.

## Workflow

### 1) Fix the migration area

1. Find all `_MalinovStation` job prototypes that the task concerns.
...
1. Collect all `roleLoadout` referenced by `_MalinovStation` roles.
2. Check each `loadoutGroup` referenced by `roleLoadout`.
3. If the group is not local to `_MalinovStation`, create a copy there and overwrite the role’s link.
4. Check all `loadout` within the group.
5. If `loadout` is not local to `_MalinovStation`, create a copy there and rewrite the link in the group.
6. Repeat until `_MalinovStation` roles have no references to vanilla or other prototypes outside the fork folder.
...
1. Make sure that all `_MalinovStation roleLoadout -> loadoutGroup -> loadout` links are local.
...
1. Copy the external `loadoutGroup` to the local `_MalinovStation` path and immediately rewrite the role’s link.
2. Copy the outer `loadout` after the group until the graph becomes completely local.
3. Give `loadout` ID in the format `MalinovLoadout<ItemPrototypeId>`.
4. Give `loadoutGroup` ID in the format `MalinovLoadoutGroup<SlotOrConcept>`, and not according to the historical copy source.
...
1. Leave the `_MalinovStation` role referring to a vanilla `loadoutGroup` instead of a local copy.
2. Copy loadout files to `Imports/...` instead of the mirror local path.
3. Give new local IDs vanilla names without the `Malinov` prefix.
4. Name files and prototypes by vanilla position, if the role has already been renamed in `_MalinovStation`.
...
```yaml
- type: loadout
  id: MalinovLoadoutKeyCardMedicalSpecialist
  equipment:
    id: KeyCardMedicalSpecialist

- type: loadoutGroup
  id: MalinovLoadoutGroupIdKeyCardMedicalSpecialist
  name: loadout-group-id
  minLimit: 1
  loadouts:
  - MalinovLoadoutKeyCardMedicalSpecialist

- type: roleLoadout
  id: JobMedicalSpecialist
  groups:
  - MalinovLoadoutGroupIdKeyCardMedicalSpecialist
```
...
```yaml
- type: loadoutGroup
  id: MalinovLoadoutGroupMedicalShoes
  name: loadout-group-shoes
  loadouts:
  - MalinovLoadoutClothingShoesColorWhite
  - MalinovLoadoutClothingShoesBootsWinterMed
...
```yaml
- type: loadoutGroup
  id: MalinovLoadoutGroupSecurityBackpack
  name: loadout-group-backpack
  loadouts:
  - MalinovLoadoutClothingBackpackSecurity
  - MalinovLoadoutClothingBackpackSatchelSecurity
  - MalinovLoadoutClothingBackpackDuffelSecurity
```
...
1. All new IDs correspond to the scheme `MalinovLoadout*` / `MalinovLoadoutGroup*` / `Job<RoleId>`.
2. All new files are located in the mirror local path and are named in `snake_case`.
3. `_MalinovStation` roles do not have references to vanilla or other loadout prototypes outside the fork folder.
