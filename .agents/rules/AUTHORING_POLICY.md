...
For every rule file in `.agents/rules/<rule-name>.md`, keep a matching bridge file:
...
A PR is incomplete if any rule exists in `.agents/rules` without a matching bridge in
`.claude/rules`.
