...
public static class PromptBuilder
{
    ...
    public static string BuildActionSelectionSystemPrompt(string category, IReadOnlyList<IAiAction> eligibleActions)
    {
        ...
        sb.AppendLine(
            "Выбор должен выполнять твоё намерение: подбор предмета не начинает разговор, а поиск не даёт отдых. " +
            "Категория служит подсказкой; если показаны действия из разных категорий, выбирай по их смыслу.");
        ...
        sb.AppendLine(
            "Для PickUpItem бери target только из списка предметов для подбора, для UseInteractable — " +
            "из списка объектов для взаимодействия, для TalkTo — из списка видимых персонажей. " +
            "Для GoToKnownLocation бери location из известных мест. Копируй названия точно. " +
            "Для SearchArea keyword может быть конкретным названием или категорией: еда, вода, люди, инструменты. " +
            "Абстрактные желания вроде отдыха или безопасного места не являются предметами поиска.");
        sb.AppendLine(
