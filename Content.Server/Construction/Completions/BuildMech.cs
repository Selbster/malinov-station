...
public sealed partial class BuildMech : IGraphAction
{
    ...
    {
        var log = IoCManager.Resolve<ILogManager>().RootSawmill; // Malinov added - retain the original log channel.
        if (!entityManager.TryGetComponent(uid, out ContainerManagerComponent? containerManager))
        {
            log.Warning($"Mech construct entity {uid} did not have a container manager! Aborting build mech action."); // Malinov edit
            return;
        }
        ...
        {
            log.Warning($"Mech construct entity {uid} did not have the specified '{Container}' container! Aborting build mech action."); // Malinov edit
            return;
        }

        if (container.ContainedEntities.Count != 1)
        {
            log.Warning($"Mech construct entity {uid} did not have exactly one item in the specified '{Container}' container! Aborting build mech action."); // Malinov edit
        }
        ...
        {
            log.Warning($"Mech construct entity {uid} had an invalid entity in container \"{Container}\"! Aborting build mech action."); // Malinov edit
            return;
        }
