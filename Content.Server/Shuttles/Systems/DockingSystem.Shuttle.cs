...
public sealed partial class DockingSystem
{
    ...
    private bool ValidSpawn(Entity<MapGridComponent> gridEntity, Matrix3x2 matty, Angle angle, FixturesComponent shuttleFixturesComp, bool isMap)
    {
        ...
        foreach (var fix in shuttleFixturesComp.Fixtures.Values)
        {
            ...
            if (isMap)
            {
                var localTiles = _mapSystem.GetLocalTilesIntersecting(gridEntity.Owner, gridEntity.Comp, aabb); // Malinov edit - equivalent current map API.

                while (localTiles.MoveNext(out var tile))
                {
                    var anchoredEnumerator = _mapSystem.GetAnchoredEntities(gridEntity.Owner, gridEntity.Comp, tile.GridIndices); // Malinov edit - equivalent current map API.
