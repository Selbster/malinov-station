...
Этап 0: скелет картриджа (прототип, компонент, система, UIFragment, локаль en/ru, тест).
...
3. Server (`MalinovMessengerCartridgeSystem`):
   ...
   - Анонс присутствия: broadcast `announce` {sender_name = PdaComponent.OwnerName} при активации;
     приём анонсов → кэш «имя → адрес» в компоненте картриджа (TTL ~30 сек, обновляется свежими анонсами).
...
1. Спавнить два PassengerPDA рядом (range 500 покрывает), установить и активировать программу на обоих
