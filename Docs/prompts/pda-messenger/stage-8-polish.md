...
После подготовки изменений и отчёта ОСТАНОВИСЬ. Коммит, push и мерж в `alpha-master` выполняет человек
ПОСЛЕ плейтеста. Тебе запрещено выполнять `git commit`, merge, rebase поверх alpha-master или push
без явного указания.
