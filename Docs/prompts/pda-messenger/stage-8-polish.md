...
Этапы 0–7 завершены и закоммичены: скелет; каталог из записей станции; отправка через
сервер-ретранслятор; история; сервер-релей; уведомления; антиспам/mute; админ-логи. Проверь `Docs/messenger-rollout.md`
