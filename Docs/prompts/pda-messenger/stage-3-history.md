...
Этапы 0–2: скелет программы; каталог экипажа из записей станции; отправка сообщений через
сервер-ретранслятор с анонсами присутствия и временной лентой строк текущей сессии. Если чего-то нет — СТОП.

## 🎯 ЦЕЛЬ ЭТАПА 3

Структурированная история переписки: по каждому контакту хранится упорядоченный список сообщений
(входящие/исходящие, время). История привязана к аккаунту (ID-карте через `MalinovMessengerAccountComponent`),
поэтому переживает деактивацию/активацию программы, закрытие КПК и перенос ID-карты в другой КПК,
...
1. Shared:
   ...
   - Заменить временную ленту этапа 2 на структуру: словарь «имя контакта → List<MalinovMessengerMessage>»
      в компоненте аккаунта `MalinovMessengerAccountComponent` на ID-карте (серверная память раунда).
      Для программы без ID-карты можно использовать локальный fallback внутри сессии.
