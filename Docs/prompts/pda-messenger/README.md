...
Пакет самодостаточных промптов для пошаговой реализации картриджа-мессенджера для КПК.
...
|---|---|---|---|
| [stage-0-skeleton.md](stage-0-skeleton.md) | 0 | Скелет картриджа: прототип, пустой UI, тест | — |
...
- Прототипы: `MalinovMessengerCartridge`, `MalinovMessengerServer`;
...
- CVar'ы: `malinov.messenger.enabled`, `.history_per_contact`, `.relay_timeout_sec`,
...
- Каталог контактов — из записей станции (StationRecords/CrewManifest), онлайн-статус — по анонсам картриджей.
